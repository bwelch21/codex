...
const ACCEPTED_FILE_TYPES = ['image/jpeg', 'image/jpg', 'image/png', 'application/pdf'] as const;
...
export function PhotoUpload({
  ...
  const getStatusColor = () => {
    switch (state.status) {
      ...
      default:
        return isDragOver ? 'border-primary-500 bg-primary-50' : 'border-neutral-300 bg-white';
    }
  };
    ...
    <div className="w-full">
        ...
        <div className="flex flex-col items-center justify-center p-8 text-center">
          {state.file && state.preview ? (
            <div className="w-full">
              ...
              <p className="text-sm font-medium text-neutral-900">{state.file.name}</p>
            ...
            <div className="w-full">
              ...
              {state.status === 'uploading' ? (
                <div>
                  <p className="text-base font-medium text-neutral-900 mb-2">Processing your file...</p>
                  <p className="text-sm text-neutral-600">Please wait while we prepare your image</p>
                </div>
              ) : (
                <div>
      ...
      {state.error && (
        <div className="mt-3 rounded-lg bg-error-50 border border-error-200 p-3">
          <div className="flex items-start">
            <svg className="h-5 w-5 text-error-500 mt-0.5 mr-2 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      {state.status === 'success' && state.file && (
        <div className="mt-3 rounded-lg bg-success-50 border border-success-200 p-3">
          <div className="flex items-start">
            <svg className="h-5 w-5 text-success-500 mt-0.5 mr-2 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <div>
              <h3 className="text-sm font-medium text-success-800">File Ready</h3>
              <p className="text-sm text-success-700 mt-1">
                Your menu image is ready to be analyzed for allergen information.
              </p>
            </div>
          </div>
        </div>
